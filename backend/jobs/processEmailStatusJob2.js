...
async function processEmailStatusJob2(job) {
  console.log('⏳ Job: Checking matchLink status for emailsent2...');

  try {
    // Step 1: Get all uniqueIds in emailsent2 with pending status
    ...
    for (const record of pendingUniqueIds) {
      ...
      // Step 2: Get all VerificationUpload_com rows for this uniqueId
      ...
      if (hasCompleted) {
        // Step 4: Update emailsent2 status to completed
        
       
        console.log(`✅ ${uniqueId} marked as completed in emailsent2`);
    ...
    return { success: true, message: 'Email status check completed for emailsent2' };
  } catch (error) {
...
module.exports = { processEmailStatusJob2 };