...
const { processLinkedinQueue } = require('./queues/processLinkedinQueue');


app.post(
  "/process-linkedin-upload",
  upload.single("file"),
  async (req, res) => {
    try {
      const email = req.headers["user-email"] || req.body.userEmail;
      ...
      await processLinkedinQueue.add('process-linkedin', {
        email,
...
// app.get('/processing-status/:email', async (req, res) => {
//   try {
//     const { email } = req.params;
    
//     // Get the latest processing status for this user
//     const latestJob = await emailsent.findOne({
//       where: { email },
//       order: [['createdAt', 'DESC']]
//     });
    
//     if (!latestJob) {
//       return res.json({ status: 'no_jobs' });
//     }
    
//     // Get associated links separately
//     const links = await Link.findAll({
//       where: { 
//         uniqueId: latestJob.uniqueId,
//         email: email
//       },
//       attributes: ['matchedCount', 'totallink', 'fileName', 'uniqueId']
//     });
    
//     // Calculate total matchedCount from all links
//     const matchedCount = links.reduce((sum, link) => sum + (link.matchedCount || 0), 0);
//     const totallink = links[0]?.totallink || 0;
//     const fileName = links[0]?.fileName || 'Unknown';
...
//       updatedCredits: latestJob.updatedCredits // Add this if you store it in emailsent
//     });
...
//               if (link.clean_link) {
//                 // Only process if clean_link exists
...
const { silentRefreshQueue } = require('./queues/silentRefreshQueue');


app.post('/trigger-silent-refresh', auth, async (req, res) => {
  try {
    const email = req.headers['user-email'] || req.body.userEmail;
    ...
    // Add job to queue
    await silentRefreshQueue.add('silent-refresh', {
      email,
      token,
    }, {
      attempts: 1,
    ...
    res.json({
      message: 'Silent refresh job enqueued',
      status: 'processing',
    });
  } catch (err) {
    console.error('Error enqueuing silent refresh job:', err);
    res.status(500).json({
      error: 'Failed to enqueue silent refresh job',
      details: err.message,
    });
  }
});
...
app.get("/api/check-file-processing", auth, async (req, res) => {
  try {
    ...
    if (user.isProcessingFile === true) {
      return res
        .status(404)
        .json({ message: "File is currently being processed" });
    }
...
//     // Process records using cursor-based pagination (more efficient than offset)
//     let hasMoreRecords = true;
...
//       await processBatch(tempRecords);

//       lastProcessedId = tempRecords[tempRecords.length - 1].id;
//     }

//     console.log(`✅ Sync completed in ${(Date.now() - jobStartTime) / 1000}s. Stats:
//       - Processed: ${processedCount}
//       - Skipped: ${skippedCount}
//       - Deleted: ${deletedCount}
//     `);
...
// async function processSingleRecord(temp) {
//   const {
//     id,
//     uniqueId,
//     linkedin_link_id,
...
//     // Determine status
//     let status = "pending";
//     const hasValidMobile = mobile_number && mobile_number !== "N/A";
//     const hasValidMobile2 = mobile_number_2 && mobile_number_2 !== "N/A";

//     if (hasValidMobile || hasValidMobile2) {
//       status = "completed";
//     }

//     // Update Link record
//     const [updated] = await Link.update({
//       mobile_number: hasValidMobile ? mobile_number : null,
//       mobile_number_2: hasValidMobile2 ? mobile_number_2 : null,
...
// console.log('⏰ TempLinkMobile sync job scheduled to run every 3 minutes');
...
const { scheduleEmailStatusChecks } = require('./schedulers/emailStatusScheduler');

// Start the email status check scheduler
scheduleEmailStatusChecks();


const { scheduleProcessingCleanup } = require('./schedulers/processingCleanupScheduler');

// Start the processing cleanup scheduler
scheduleProcessingCleanup();
...
// cron.schedule('*/3 * * * *', checkAndUpdateEmailStatus);
...
app.post("/send-upload-notification", auth, async (req, res) => {
  try {
    const { email, fileName, totalLinks, matchCount, creditsDeducted } =
      req.body;
...
app.post("/api/verify-payment", async (req, res) => {
  try {
    ...
    if (!email || !creditAmount || creditAmount <= 0) {
      return res
        .status(400)
        .json({ message: "Invalid email or credit amount" });
    }
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      // Add job to queue and return immediately
      await processLinkedinQueue.add('process-verification', {
        email,
        filePath: req.file.path,
        originalFileName: req.file.originalname,
        processCredits
      ...
      // Return immediate response
      res.json({
        message: "File uploaded and verification processing started",
        status: "processing"
      });
...
//           if (
//             /linkedin\.com\/(sales\/lead|sales\/people)\/ACw|ACo|acw|acw/i.test(
//               link
//             )
//           ) {
//             remark = "Sales Navigator Link";
...
//             { where: {
//               uniqueId,
//               clean_link: { [Op.ne]: null }, // Only count successfully matched links
//             },
//            }
//           );
...
app.get("/get-verification-links", async (req, res) => {
  try {
    const email = req.headers["user-email"];
    if (!email)
      return res.status(400).json({ error: "Email required in headers" });

    const userLinks = await VerificationUpload.findAll({
      where: { email },
      order: [["date", "DESC"]], // most recent first
    });
...
app.post("/api/deduct-credits_v", auth, async (req, res) => {
  try {
    ...
    if (!userEmail || !credits || credits <= 0) {
      return res
        .status(400)
        .json({ error: "Invalid credit deduction request" });
    }
    ...
    await VerificationUpload.update(
      { creditsUsed: credits, remainingCredits: user.credits },
      { where: { uniqueId } }
    );
...
app.delete(
  "/api/delete-verification-uploads/:uniqueId",
  auth,
  async (req, res) => {
    try {
      const { uniqueId } = req.params;
...
app.post("/api/send-verification-confirmation/link", auth, async (req, res) => {
  const { email, uniqueId, totalLinks, pendingCount, creditCost, initiatedBy } =
    req.body;
...
//       // Process batch with controlled concurrency
//       await processVerificationBatch(tempRecords);
...
//     const jobDuration = (Date.now() - jobStartTime) / 1000;
//     console.log(`✅ Sync completed in ${jobDuration}s. Stats:
//       - Processed: ${processedCount}
//       - Marked as completed: ${markedCompletedCount}
//       - Deleted: ${deletedCount}
//       - Skipped: ${skippedCount}
...
//     if (jobDuration > 240) { // 4 minutes threshold
//       console.warn('⚠️ Job is taking too long, consider optimizing further');
//     }
...
// // Process batch with controlled concurrency
// async function processVerificationBatch(records) {
//   const BATCH_CONCURRENCY = 2; // Lower concurrency for complex operations

//   for (let i = 0; i < records.length; i += BATCH_CONCURRENCY) {
//     const batch = records.slice(i, i + BATCH_CONCURRENCY);

//     await Promise.allSettled(
//       batch.map(record => processSingleVerificationRecord(record))
//     );
...
// async function processSingleVerificationRecord(tempData) {
//   const { id, link_id, uniqueId } = tempData;

//   // Skip if required fields are missing
//   if (!link_id || !uniqueId) {
//     console.log(`⚠️ Skipping record ${id}: Missing link_id or uniqueId`);
//     skippedCount++;
//     return;
//   }
...
//       head_title: tempData.head_title || null,
//       head_location: tempData.head_location || null,
//       title_1: tempData.title_1 || null,
//       company_1: tempData.company_1 || null,
//       company_link_1: tempData.company_link_1 || null,
//       exp_duration: tempData.exp_duration || null,
//       exp_location: tempData.exp_location || null,
//       job_type: tempData.job_type || null,
//       title_2: tempData.title_2 || null,
//       company_2: tempData.company_2 || null,
//       company_link_2: tempData.company_link_2 || null,
//       exp_duration_2: tempData.exp_duration_2 || null,
//       exp_location_2: tempData.exp_location_2 || null,
//       job_type_2: tempData.job_type_2 || null,
//       final_remarks: tempData.final_remarks || null,
//       list_contacts_id: tempData.list_contacts_id || null,
//       url_id: tempData.url_id || null,
//       last_sync: new Date()
//     };
...
//     if (updated > 0) {
//       processedCount++;
//       console.log(`✓ Updated VerificationUpload for link_id: ${link_id}, uniqueId: ${uniqueId}`);
...
//         markedCompletedCount++;
//         deletedCount++;
//         console.log(`🗑️ Deleted VerificationTemp record ${id}`);
//       }
//     } else {
//       skippedCount++;
//       console.log(`⚠️ No matching record found for link_id: ${link_id}`);
//     }
...
//   } catch (recordError) {
//     if (transaction) await transaction.rollback();
//     console.error(`⚠️ Error processing record ${id}:`, recordError.message);
//   }
// }
...
// cron.schedule('*/3 * * * *', checkAndUpdateEmailStatus1);
// console.log('⏰ Email status check job scheduled to run every 10 minutes');
...
//     for (const user of staleUsers) {
//       await user.update({
//         isProcessingFile1: false,
//         processingStartTime1: null,
//       });
//     }
//   } catch (error) {
//     console.error("Error in processing cleanup job:", error);
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      console.error("Upload error:", err);
      if (req.file?.path) fs.unlinkSync(filePath);
      res.status(500).json({ error: "Upload failed", details: err.message });
    }
  }
);
...
app.post(
  "/con-upload-excel-verification-com",
  upload.single("file"),
  async (req, res) => {
    try {
      const email = req.headers["user-email"] || req.body.userEmail;
      ...
      // Add job to queue and return immediately
      await processLinkedinQueue.add('process-verification-com', {
        email,
        filePath: req.file.path,
        originalFileName: req.file.originalname,
        processCredits
      ...
      // Return immediate response
      res.json({
        message: "File uploaded and company verification processing started",
        status: "processing"
      });
...
app.get("/get-verification-links-com", async (req, res) => {
  try {
    ...
    const userLinks = await VerificationUpload_com.findAll({
      where: { email },
      order: [["date", "DESC"]], // most recent first
    });
...
// Add this to your backend routes (e.g., in your Express server)
app.delete(
  "/api/delete-verification-uploads-com/:uniqueId",
  auth,
  async (req, res) => {
    try {
      const { uniqueId } = req.params;
...
//     // Process records using cursor-based pagination
//     while (hasMoreRecords) {
//       const tempRecords = await VerificationTemp_com.findAll({
//         where: {
//           id: { [Op.gt]: lastProcessedId }
//         },
//         limit: 100, // Reduced batch size for better performance
//         order: [['id', 'ASC']],
//         raw: true // Faster data retrieval
//       });
...
//       // Process batch with controlled concurrency
//       await processCompanyBatch(tempRecords);
...
// // Process batch with controlled concurrency
// async function processCompanyBatch(records) {
//   const BATCH_CONCURRENCY = 2; // Lower concurrency for database-intensive operations

//   for (let i = 0; i < records.length; i += BATCH_CONCURRENCY) {
//     const batch = records.slice(i, i + BATCH_CONCURRENCY);

//     await Promise.allSettled(
//       batch.map(record => processSingleCompanyRecord(record))
//     );
...
// async function processSingleCompanyRecord(tempData) {
//   const { id, link_id, uniqueId } = tempData;
...
//     // Update main table - use atomic update to avoid overwriting existing data unnecessarily
//     const [updated] = await VerificationUpload_com.update(updateData, {
//       where: {
//         uniqueId: uniqueId,
//         link_id: link_id
...
//     if (updated > 0) {
//       processedCount++;
//       console.log(`✓ Updated VerificationUpload_com for link_id: ${link_id}`);
...
//         markedCompletedCount++;
//         deletedCount++;
//         console.log(`🗑️ Deleted temp record ${id}`);
//       }
//     } else {
//       skippedCount++;
//       console.log(`⚠️ No matching record found for link_id: ${link_id}`);
...
//     errorCount++;
//     if (transaction) await transaction.rollback();
//     console.error(`❌ Error processing record ${id}:`, recordError.message);
//   }
// }
...
//       html: `
//         <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
//           <h2 style="color: #2563eb;">B2B LinkedIn Company Details Completed</h2>
//           <p>All Company Details processes for ${uniqueId} have been completed.</p>

//           <div style="background-color: #f3f4f6; padding: 15px; border-radius: 8px; margin: 15px 0;">
//             <h3 style="margin-top: 0; color: #1f2937;">LinkedIn Company Details</h3>
//             <p><strong>File UniqueId:</strong> ${uniqueId}</p>
//           </div>
...
// cron.schedule('*/3 * * * *', checkAndUpdateEmailStatus2);
// console.log('⏰ Email status check job scheduled to run every 10 minutes');
...
//     for (const user of staleUsers) {
//       await user.update({
//         isProcessingFile2: false,
//         processingStartTime2: null,
//       });
//     }
//   } catch (error) {
//     console.error("Error in processing cleanup job:", error);
...
// Email confirmation endpoint
app.post(
  "/api/send-verification-confirmation/company",
  auth,
  async (req, res) => {
    const {
      email,
  ...
  try {
    ...
    const links = await Link.findAll({
      attributes: [
        ...
        [
          sequelize.fn("MIN", sequelize.col("remainingCredits")),
          "remainingCredits",
        ],
        [
          sequelize.fn("MIN", sequelize.col("creditDeducted")),
          "creditDeducted",
        ],
        [
          sequelize.fn(
...
app.post("/verify-payment", (req, res) => {
  try {
    ...
    const expectedSignature = crypto
      .createHmac(
        "sha256",
        process.env.RAZORPAY_KEY_SECRET || "NnBbVvCcXxZzSsDdFfGgHhJjKkLl"
      )
...
// async function updatePendingLinks() {
//   try {
//     // Connect to the database
//     await sequelize.authenticate();
//     console.log('Database connected successfully.');

//     // Update all records with final_status 'pending' to 'completed'
//     const [updatedCount] = await VerificationUpload.update(
//       { final_status: 'completed' },
//       { 
//         where: { 
...
//     console.log(`Successfully updated ${updatedCount} records from 'pending' to 'completed'.`);
...
app.get("/api/verifications/minimal-report_V", auth, async (req, res) => {
  try {
    ...
    // Get status details for each uniqueId to determine final_status
    const statusDetails = await VerificationUpload.findAll({
      attributes: ["uniqueId", "final_status"],
      where: {
        uniqueId: verifications.map((verification) => verification.uniqueId),
      },
