...
const UserList = () => {
  ...
  const fetchUsers = async () => {
    try {
      const response = await fetch(
        `http://13.200.242.61:8000/users/created-by/${userEmail}`
      );
  ...
  const fetchUserCredits = async () => {
    try {
      const response = await fetch(
        `http://13.200.242.61:8000/users/credits/${encodeURIComponent(userEmail)}`
      );
      if (!response.ok) throw new Error(`Error: ${response.statusText}`);
  ...
  const handleCreditTransfer = async (email, existingCredits, isAdding) => {
    ...
    try {
      const response = await fetch(
        "http://13.200.242.61:8000/transactions/update-credits",
        {
          method: "PATCH",
