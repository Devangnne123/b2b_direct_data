...
const AllAdmin = () => {
  ...
  const fetchUsersWithCredits = async () => {
    ...
    try {
      const response = await fetch("http://3.6.160.211:8000/users/getAllAdmin");
  ...
  const handleUpdateCredits = async (userEmail, newCredits) => {
    ...
    try {
      const response = await fetch(
        "http://3.6.160.211:8000/users/update-credits",
        {
          method: "PATCH",
      ...
      const transactionResponse = await fetch(
        "http://3.6.160.211:8000/super-admin/assign-credits",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
