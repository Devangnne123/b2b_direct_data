...
const CheckoutPage = () => {
  ...
  const fetchUserCredits = async (email) => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/user/${email}`, {
...
const handleSuccess = async (details) => {
  ...
  try {
    ...
    const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/payments/update-credits`, {
      method: 'POST',
      headers: {
