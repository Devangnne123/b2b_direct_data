  ...
  useEffect(() => {
   ...
   useEffect(() => {
    const fetchAdminCreditCost = async () => {
      try {
        const response = await axios.get("http://13.200.242.61:8000/users/getAllAdmin");
...
//   try {
//     const response = await axios.get('http://13.200.242.61:8000http://13.200.242.61:8000/credit-cost', {
//       params: { email } // This will create http://13.200.242.61:8000/credit-cost?email=user@example.com
//     });
...
// };
  ...
  const fetchCredits = async (email) => {
    try {
      const res = await axios.get(`http://13.200.242.61:8000/api/user/${email}`);
  ...
  const fetchUserLinks = async (email) => {
    setLoading(true);
    try {
      const res = await axios.get("http://13.200.242.61:8000/get-links", {
 ...
 const handleUpload = async () => {
  ...
  try {
    const res = await axios.post(
      "http://13.200.242.61:8000/upload-excel",
      formData,
  ...
  const confirmUpload = async () => {
    ...
    try {
      const creditRes = await axios.post(
        "http://13.200.242.61:8000/api/upload-file",
        {
  ...
  const cancelUpload = async () => {
    ...
    try {
      await axios.delete(
        `http://13.200.242.61:8000/cancel-upload/${pendingUpload.uniqueId}`
      );
                  ...
                  <div className="upload-section">
                    <div className="history-table">
                      ...
                      <div className="coming-soon-message text-center py-10">
                        <h2 className="text-2xl font-semibold text-gray-700">📢 LinkedIn Verification Coming Soon!</h2>
                        <p className="text-gray-500 mt-2">
                          We’re building powerful tools to help you validate LinkedIn profiles faster. Stay tuned!
                        </p>
                      </div>
                    </div>
                  </div>
