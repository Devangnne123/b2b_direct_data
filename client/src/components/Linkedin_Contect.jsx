...
//   try {
//     const response = await axios.post('${import.meta.env.VITE_API_BASE_URL}${import.meta.env.VITE_API_BASE_URL}/credit-cost', {
//       params: { email } // This will create ${import.meta.env.VITE_API_BASE_URL}/credit-cost?email=user@example.com
//     });
...
// };
  ...
  const fetchCredits = async (email) => {
    try {
      const res = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/user/${email}`, {
  ...
  const fetchUserLinks = async (email) => {
    setLoading(true);
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/bulklookup/get-links`,{
