  ...
  useEffect(() => {
   ...
   useEffect(() => {
    const fetchAdminCreditCost = async () => {
      try {
        const response = await axios.get("http://localhost:3880/users/getAllAdmin");
...
//   try {
//     const response = await axios.get('http://localhost:3880/api/credit-cost', {
...
// };
 ...
 const handleUpload = async () => {
  ...
  const cancelUpload = async () => {
    ...
    try {
      await axios.delete(
        `http://localhost:3880/cancel-upload/${pendingUpload.uniqueId}`
      );
