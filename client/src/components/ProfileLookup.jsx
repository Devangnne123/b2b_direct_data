...
const handleSearch = async () => {
  ...
  try {
    const res = await axios.get(`http://localhost:3880/api/links/search-match?matchLink=${matchLink}`);
    ...
    if (res.data.result) {
      ...
      await fetch(`http://localhost:3880/bulkUpload/add`, {
        method: "POST",
        headers: {
  ...
  const updateUserCredits = async (newCredits) => {
    try {
      await fetch(`http://localhost:3880/users/update-credits`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${user.token}`,
  ...
  const handleSearch0 = async () => {
    ...
    try {
      setIsLoading(true);
      const apiUrl = `http://localhost:3880/mobileEnrichments/mobileEnrichment/single/${encodeURIComponent(
        linkedinLink
      )}`;
