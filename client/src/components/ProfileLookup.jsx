...
const handleSearch = async () => {
  ...
  try {
    const res = await axios.get(`http://13.203.218.236:3005/api/links/search-match?matchLink=${matchLink}`);
    ...
    if (res.data.result) {
      ...
      await fetch(`http://13.203.218.236:3005/bulkUpload/add`, {
        method: "POST",
        headers: {
  ...
  const fetchUserCredits = async () => {
    try {
      const response = await fetch(`http://13.203.218.236:3005/users/user`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${user.token}`,
  ...
  const updateUserCredits = async (newCredits) => {
    try {
      await fetch(`http://13.203.218.236:3005/users/update-credits`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
  ...
  const handleSearch0 = async () => {
    ...
    try {
      setIsLoading(true);
      const apiUrl = `http://13.203.218.236:3005/mobileEnrichments/mobileEnrichment/single/${encodeURIComponent(
        linkedinLink
      )}`;
