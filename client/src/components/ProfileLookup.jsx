...
const handleSearch = async () => {
  ...
  try {
    const res = await axios.get(`http://13.200.242.61:8000/api/links/search-match?matchLink=${matchLink}`);
    ...
    if (res.data.result) {
      ...
      await fetch(`http://13.200.242.61:8000/bulkUpload/add`, {
        method: "POST",
        headers: {
  ...
  const fetchUserCredits = async () => {
    try {
      const response = await fetch(`http://13.200.242.61:8000/users/user`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${user.token}`,
  ...
  const updateUserCredits = async (newCredits) => {
    try {
      await fetch(`http://13.200.242.61:8000/users/update-credits`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${user.token}`,
  ...
  const handleSearch0 = async () => {
    ...
    try {
      setIsLoading(true);
      const apiUrl = `http://13.200.242.61:8000/mobileEnrichments/mobileEnrichment/single/${encodeURIComponent(
        linkedinLink
      )}`;
